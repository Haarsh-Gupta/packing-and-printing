...
export default function Services() {
    ...
    const fetchServices = async () => {
        ...
        try {
            ...
            const data = await api<Service[]>("/admin/services/?limit=1000");
            setServices(data);
        } catch (e) {
            console.error(e);
        } finally {
    ...
    const confirmDelete = async () => {
        ...
        try {
            await api(`/admin/services/${deleteTarget.id}`, { method: "DELETE" });
            fetchServices();
                                ...
                                <tr key={service.id} onClick={() => navigate(`/services/${service.slug}`)} className="group hover:bg-slate-50 dark:hover:bg-[#171f33]/80 transition-colors cursor-pointer">
                                    ...
                                    <td className="px-6 py-5">
                                        ...
                                        <div className="flex flex-wrap gap-1.5 max-w-[200px]">
                                            {service.sub_services?.slice(0, 3).map((ss) => (
                                                <span key={ss.id} className="bg-slate-100 dark:bg-[#171f33] border border-slate-200 dark:border-[#434655]/30 text-slate-700 dark:text-[#dae2fd] text-[9px] font-medium px-2 py-0.5 rounded truncate max-w-[100px]" title={ss.name}>
                                                    {ss.name}
                                                </span>
                                            ))}
                                            {(service.sub_services?.length || 0) > 3 && (
                                    ...
                                    <td className="px-6 py-5 text-right">
                                        <div className="inline-flex items-center gap-1.5">
                                            ...
                                            <button onClick={(e) => handleDeleteClick(e, service)} className="p-2 opacity-0 group-hover:opacity-100 hover:bg-[#ffb4ab]/10 rounded-lg transition-all text-slate-600 dark:text-[#c3c5d8] hover:text-[#ffb4ab]">
                                                <Trash2 size={16} />
                                            </button>
                ...
                <DialogContent className="sm:max-w-md bg-slate-50 dark:bg-[#0b1326] border border-slate-200 dark:border-[#434655]/30 text-slate-900 dark:text-[#dae2fd] shadow-2xl font-['Inter']">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-xs text-slate-600 dark:text-[#c3c5d8] mt-2">
                            This action cannot be undone. This will permanently delete the service <strong className="text-slate-900 dark:text-white">{deleteTarget?.name}</strong> and all of its associated sub-services and configurations.
                        </DialogDescription>
                    </DialogHeader>
