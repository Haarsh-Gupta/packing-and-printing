...
export default function Reviews() {
    ...
    return (
        <div className="flex flex-col h-full font-['Inter'] bg-[#0b1326] text-[#dae2fd] px-2 pb-12">
            ...
            <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6 mb-8">
                <div>
                    <nav className="flex items-center gap-2 text-[10px] font-bold text-[#adc6ff] mb-2 tracking-widest uppercase">
                        <span>Feedback</span>
                        <span>/</span>
                        <span className="text-[#c3c5d8]/60">Moderation</span>
                    </nav>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="flex p-1 bg-[#131b2e] border border-[#434655]/30 rounded-lg">
                        <button
                    ...
                    <div className="relative">
                        ...
                        <input 
                            ...
                            className="pl-9 pr-3 h-10 w-48 text-xs font-mono text-[#adc6ff] bg-[#131b2e] border border-[#434655]/40 rounded-lg focus:border-[#adc6ff] outline-none transition-colors placeholder:text-[#434655]/70"
            ...
            {/* Filtering Controls */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-[#131b2e] p-5 rounded-2xl border border-[#434655]/30 mb-8">
                <div className="space-y-2">
                    <label className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-[#c3c5d8]">
                        <Layers size={14} className="text-[#8d90a1]" /> Parent Category
                    </label>
                    <select 
                        value={filters.parent_id} 
                        onChange={handleParentChange}
                        className="w-full h-11 px-3 bg-[#0b1326] border border-[#434655]/40 rounded-lg text-sm font-bold text-[#dae2fd] focus:border-[#adc6ff] outline-none transition-colors"
                    >
                ...
                <div className="space-y-2">
                    <label className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-[#c3c5d8]">
                        <Tags size={14} className="text-[#8d90a1]" /> Specific Variant Link
                    </label>
                    <select 
                        value={filters.sub_id} 
                        onChange={handleSubChange} 
                        disabled={filters.parent_id === "all"}
                        className="w-full h-11 px-3 bg-[#0b1326] border border-[#434655]/40 rounded-lg text-sm font-bold text-[#dae2fd] focus:border-[#adc6ff] outline-none transition-colors disabled:opacity-50"
                    >
            ...
            {/* Content Area */}
            <div className="bg-[#131b2e] rounded-2xl border border-[#434655]/20 flex flex-col flex-1 overflow-hidden min-h-[400px]">
                {loading && !reviewData ? (
                    <div className="flex-1 flex flex-col items-center justify-center py-20">
                        <Loader2 size={32} className="animate-spin text-[#adc6ff] mb-4" />
                        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#c3c5d8]">Accessing Logs...</p>
                    </div>
                ) : !reviewData || reviewData.reviews.length === 0 ? (
                    <div className="flex-1 flex flex-col items-center justify-center py-24 text-center">
                        <div className="w-16 h-16 rounded-2xl bg-[#0b1326] border border-[#434655]/30 flex items-center justify-center mb-4 text-[#434655]">
                            <MessageSquare size={32} />
                        </div>
                        <h3 className="font-bold text-sm text-[#dae2fd] uppercase tracking-widest">No Log Entries</h3>
                        <p className="text-xs text-[#8d90a1] mt-2 max-w-sm">Adjust query parameters or target a different vector space.</p>
                        <button onClick={resetFilters} className="mt-6 px-4 py-2 border border-[#434655]/40 hover:border-[#adc6ff]/50 rounded text-[10px] font-bold tracking-widest uppercase text-[#adc6ff] transition-colors">
                            Clear Parameters
                        </button>
                    </div>
                ) : (
                    <div className="flex flex-col flex-1">
                        <div className="px-6 py-4 bg-[#060e20]/50 border-b border-[#434655]/20 flex items-center justify-between">
                            <span className="text-[10px] font-bold uppercase tracking-widest text-[#8d90a1]">
                                Rendering {reviewData.reviews.length} of {reviewData.total} items
                            </span>
                            
                            <div className="flex items-center gap-2">
                                <button
                                    className="p-1.5 text-[#c3c5d8] bg-[#0b1326] border border-[#434655]/40 rounded hover:text-[#adc6ff] hover:border-[#adc6ff]/50 disabled:opacity-30 disabled:pointer-events-none transition-colors"
                                    disabled={filters.skip === 0}
                                    onClick={() => setFilters(f => ({ ...f, skip: Math.max(0, f.skip - f.limit) }))}
                                >
                                    <ChevronLeft size={16} />
                                </button>
                                <span className="text-[10px] font-bold text-[#adc6ff] px-3 py-1 bg-[#1f70e3]/10 border border-[#1f70e3]/20 rounded font-mono">
                        ...
                        <div className="divide-y divide-[#434655]/10 flex-1 overflow-y-auto custom-scrollbar">
                            {reviewData.reviews.map((r) => (
                                <div key={r.id} className="p-6 flex gap-6 hover:bg-[#171f33]/80 transition-colors group">
                                    <div className="w-12 h-12 rounded-xl bg-[#0b1326] flex items-center justify-center shrink-0 border border-[#434655]/30 overflow-hidden">
                                    ...
                                    <div className="flex-1 min-w-0 space-y-3">
                                        <div className="flex items-start justify-between">
                                            <div>
                                                <div className="flex items-center gap-3">
                                                    <h3 className="font-extrabold text-[#dae2fd] text-sm">{r.user?.name || "Ghost Node"}</h3>
                                                    {r.is_verified && (
                                                ...
                                                <div className="flex items-center gap-4 mt-2">
                                                    ...
                                                    <span className="text-[10px] text-[#8d90a1] font-bold flex items-center gap-1.5 uppercase tracking-wider">
                                        ...
                                        <p className="text-[13px] leading-relaxed text-[#c3c5d8] italic bg-[#0b1326] p-4 rounded-xl border border-[#434655]/20">
                                            "{r.comment}"
                                        </p>

                                        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#060e20] rounded-lg border border-[#434655]/30">
                                            <Package size={14} className="text-[#434655]" />
                                            <span className="text-[10px] font-bold text-[#c3c5d8] uppercase tracking-wider">
                                                {r.product?.name || r.service?.name || "Unlinked Asset"}
                                                <span className="mx-2 text-[#434655]">|</span>
                                                <span className="text-[#8d90a1] font-mono">ID_{r.product? r.product.id : r.service?.id}</span>
                                            </span>
                                        </div>
                                    </div>
                                </div>
            ...
            <div className="mt-6 p-4 bg-[#f59e0b]/10 rounded-xl border border-[#f59e0b]/20 flex gap-4 items-start">
                ...
                <div>
                    ...
                    <p className="text-xs text-[#c3c5d8]/80 mt-1 leading-relaxed">
