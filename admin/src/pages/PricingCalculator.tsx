        ...
        <div className="flex flex-col h-full font-['Inter'] bg-[#0b1326] text-[#dae2fd] px-2 pb-12 animate-fade-in">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 gap-4">
                <div>
                    <nav className="flex items-center gap-2 text-[10px] font-bold text-[#adc6ff] mb-2 tracking-widest uppercase">
                        <span>Tools</span>
                        <span>/</span>
                        <span className="text-[#c3c5d8]/60">Estimator</span>
                    </nav>
                    <h1 className="text-3xl font-extrabold tracking-tight text-[#dae2fd] m-0 flex items-center gap-3">
                        <Calculator className="text-[#1f70e3]" size={28} /> Pricing Sandbox
                    </h1>
                    <p className="text-xs text-[#c3c5d8] mt-1 m-0">Simulate payload configurations and rate modifications in an isolated environment.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6 items-start">
                ...
                <div className="flex flex-col gap-6">
                    <div className="bg-[#131b2e] rounded-2xl shadow-sm border border-[#434655]/20 flex flex-col overflow-hidden">
                        <div className="px-6 py-5 border-b border-[#434655]/20 bg-[#060e20]/50">
                            <h2 className="text-[11px] uppercase tracking-widest font-extrabold text-[#dae2fd] m-0">Entity Vector</h2>
                        </div>
                        
                        <div className="p-6 flex flex-col gap-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div className="flex flex-col gap-2">
                                    <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#8d90a1]">Class</label>
                                    <div className="relative">
                                        <select 
                                            value={selectedType} 
                                            onChange={e => handleSelectType(e.target.value)}
                                            className="w-full h-11 pl-4 pr-10 rounded-lg border border-[#434655]/40 text-sm font-bold bg-[#0b1326] text-[#dae2fd] outline-none focus:border-[#adc6ff] transition-colors appearance-none"
                                        >
                                ...
                                <div className="flex flex-col gap-2">
                                    <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#8d90a1]">Identifier</label>
                                    <div className="relative">
                                        <select 
                                            value={String(selectedId)} 
                                            onChange={e => handleSelectItem(e.target.value)} 
                                            disabled={!selectedType || loading}
                                            className="w-full h-11 pl-4 pr-10 rounded-lg border border-[#434655]/40 text-sm font-bold bg-[#0b1326] text-[#dae2fd] outline-none focus:border-[#adc6ff] transition-colors appearance-none disabled:opacity-50"
                                        >
                        ...
                        {selectedId && (
                            <div className="border-t border-[#434655]/20 bg-[#0b1326]">
                                <div className="p-6 space-y-6">
                                    <div className="flex flex-col gap-2">
                                        <label className="text-[10px] font-bold uppercase tracking-widest text-[#c3c5d8]">Base Evaluation (₹)</label>
                                        <input 
                                            type="number" 
                                            value={basePrice} 
                                            onChange={e => setBasePrice(e.target.value)} 
                                            className="w-full h-12 px-4 rounded-xl border border-[#434655]/40 bg-[#131b2e] text-lg font-mono font-extrabold text-[#adc6ff] outline-none focus:border-[#adc6ff] transition-colors"
                                        />
                                    </div>

                                    {selectedType === "product" && configSchema?.sections?.length > 0 && (
                                        <div className="flex flex-col gap-4">
                                            <label className="text-[10px] font-bold uppercase tracking-widest text-[#c3c5d8]">Configuration Parameters</label>
                                            {configSchema.sections.map((sec: any) => (
                                                <div key={sec.key} className="flex flex-col gap-3 p-5 rounded-xl border border-[#434655]/40 bg-[#171f33]/50">
                                                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                                                        <div className="flex flex-col gap-1.5">
                                                            <span className="text-[11px] font-extrabold text-[#dae2fd] leading-none uppercase tracking-widest">{sec.label}</span>
                                                            <span className="text-[9px] font-mono uppercase tracking-[0.2em]">
                                                                {sec.type === 'number_input' ? (
                                                                    <span className="text-[#8d90a1]">₹{sec.price_per_unit || 0} / UNIT</span>
                                                                ) : (
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <div className="relative">
                                                                <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[10px] font-mono text-[#c3c5d8]">₹</span>
                                                                <input 
                                                                    type="number" 
                                                                    placeholder="0" 
                                                                    value={optionFixedMarkups[sec.key] || ""} 
                                                                    onChange={e => handleFixedMarkupChange(sec.key, e.target.value)} 
                                                                    className="w-[80px] h-8 pl-6 pr-2 rounded text-[11px] font-mono font-bold bg-[#0b1326] border border-[#434655]/40 text-[#dae2fd] outline-none focus:border-[#adc6ff] transition-colors placeholder:text-[#434655]" 
                                                                />
                                                            </div>
                                                            ...
                                                            <div className="relative">
                                                                <input 
                                                                    ...
                                                                    className="w-[70px] h-8 px-2 pr-6 rounded text-[11px] font-mono font-bold bg-[#0b1326] border border-[#434655]/40 text-[#dae2fd] outline-none focus:border-[#adc6ff] transition-colors placeholder:text-[#434655] text-right" 
                                                                />
                                                                <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] font-mono text-[#c3c5d8]">%</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    {sec.type === "dropdown" || sec.type === "radio" ? (
                                                        <div className="relative w-full">
                                                            <select 
                                                                ...
                                                                className="w-full h-11 pl-4 pr-10 rounded-lg border border-[#434655]/40 text-xs font-bold bg-[#0b1326] text-[#dae2fd] outline-none focus:border-[#adc6ff] transition-colors appearance-none"
                                                            >
                                                        ...
                                                        <input 
                                                            ...
                                                            onChange={e => handleOptionChange(sec.key, e.target.value)} 
                                                            className="w-full h-11 px-4 rounded-lg border border-[#434655]/40 text-xs font-bold bg-[#0b1326] text-[#dae2fd] outline-none focus:border-[#adc6ff] transition-colors placeholder:text-[#434655]" 
                                                        ...
                                                        <input 
                                                            ...
                                                            value={selectedOptions[sec.key] || ""} 
                                                            onChange={e => handleOptionChange(sec.key, e.target.value)} 
                                                            className="w-full h-11 px-4 rounded-lg border border-[#434655]/40 text-xs font-bold bg-[#0b1326] text-[#dae2fd] outline-none focus:border-[#adc6ff] transition-colors" 
                                                        />
                                                    )}
                                                </div>
                                            ))}
                ...
                <div className="flex flex-col gap-6 sticky top-6 self-start">
                    ...
                    <div className="bg-[#1f70e3] rounded-2xl shadow-lg border border-transparent overflow-hidden text-white relative">
                        ...
                        <div className="p-8 relative z-10 flex flex-col items-center justify-center text-center gap-4">
                            <p className="text-[10px] font-extrabold text-[#adc6ff] tracking-[0.3em] uppercase flex items-center gap-2 m-0">
                                <Activity size={14} /> Live Computation
                            </p>
                            <h3 className="text-5xl font-extrabold tracking-tight font-mono text-white m-0">
                                ₹{total.toLocaleString()}
                            </h3>
                            <div className="bg-[#001a42]/30 border border-white/10 px-4 py-1.5 rounded-lg text-[11px] font-mono text-[#adc6ff] shadow-inner mt-2">
                        ...
                        <div className="p-6 bg-[#001a42]/60 flex flex-col gap-5 border-t border-white/10 relative z-10">
                            <div className="flex flex-col gap-2">
                                <label className="text-[9px] font-bold text-[#adc6ff] uppercase tracking-[0.2em] text-center">Batch Size Volume</label>
                                <input 
                                    type="number" 
                                    min="1" 
                                    value={quantity} 
                                    onChange={e => setQuantity(e.target.value)} 
                                    className="w-full h-12 text-center text-xl font-mono font-extrabold bg-[#0b1326] border border-[#1f70e3]/50 rounded-xl text-white outline-none focus:border-white transition-colors" 
                                />
                            </div>
                            
                            <div className="flex flex-col gap-3 pt-4 border-t border-[#1f70e3]/30">
                                <label className="text-[9px] font-bold text-[#adc6ff] uppercase tracking-[0.2em] text-center">Global Metric Override</label>
                                <div className="flex items-center gap-4">
                                    <input 
                                        ...
                                        className="flex-1 accent-white h-1.5 rounded-full bg-[#131b2e] appearance-none"
                                    />
                                    <div className="relative w-20 shrink-0">
                                        <input 
                                            ...
                                            value={globalMarkup} 
                                            onChange={e => setGlobalMarkup(e.target.value)} 
                                            className="w-full h-9 pl-2 pr-6 rounded-lg text-xs font-mono font-extrabold bg-[#0b1326] border border-[#1f70e3]/50 text-white outline-none text-center" 
                                        />
                                        <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-[#adc6ff]">%</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-[#f59e0b]/10 rounded-xl p-5 border border-[#f59e0b]/20 flex gap-4">
                        ...
                        <div className="flex flex-col gap-1.5 text-[#dae2fd]">
                            <p className="text-[11px] font-extrabold uppercase tracking-widest text-[#fcd34d] m-0">Isolated Environment</p>
                            <p className="text-[11px] font-medium leading-relaxed m-0 text-[#c3c5d8]">
