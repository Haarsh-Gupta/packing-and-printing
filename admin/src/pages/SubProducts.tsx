...
import { ArrowLeft, ChevronRight, Edit2, Trash2, PlusCircle, Loader2, Save, Info, Plus, X, GripVertical, Settings2 } from "lucide-react";
            ...
            <div className="flex items-end justify-between mb-8">
                <div>
                    <nav className="flex items-center gap-2 text-[10px] font-bold text-[#adc6ff] mb-2 tracking-widest uppercase">
                        ...
                        <span className="text-[#c3c5d8]/60">{product ? product.name : "Loading..."}</span>
                        <ChevronRight size={10} className="text-[#434655]" />
                        <span className="text-[#c3c5d8]/60">Variants</span>
                    </nav>
                    <div className="flex items-center gap-4">
                        <button onClick={() => navigate('/products')} className="p-2 -ml-2 hover:bg-[#131b2e] rounded-lg transition-colors text-[#c3c5d8] hover:text-[#adc6ff]">
                            <ArrowLeft size={20} />
                        </button>
                ...
                <div className="overflow-x-auto custom-scrollbar flex-1">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-[#434655]/10">
                            {loading ? (
                                <tr><td colSpan={6} className="text-center py-16 text-[#c3c5d8] font-bold tracking-widest text-[10px] uppercase"><Loader2 className="animate-spin inline mr-2" size={16} /> Syncing...</td></tr>
                            ) : subProducts.length === 0 ? (
                                <tr><td colSpan={6} className="text-center py-16 text-[#c3c5d8]/50 font-bold tracking-widest text-[10px] uppercase">No variant nodes deployed.</td></tr>
                            ) : subProducts.map((p, idx) => {
                                const typeColor = typeColors[idx % typeColors.length];
                                return (
                                    <tr key={p.id} className="group hover:bg-[#171f33]/80 transition-colors">
                                            ...
                                            <div className="flex items-center gap-4">
                                                <div className="flex -space-x-3 overflow-hidden rounded-lg relative">
                                                    {p.images && p.images.length > 0 ? p.images.slice(0, 3).map((img, i) => (
                                                        <img key={i} src={img} className="inline-block h-10 w-10 rounded-lg ring-2 ring-[#131b2e] object-cover bg-[#0b1326]" />
                                                    )) : (
                                                        <div className="inline-block h-10 w-10 rounded-lg ring-2 ring-[#131b2e] bg-[#0b1326] border border-[#434655]/30 flex items-center justify-center font-bold text-[#434655] text-[9px]">IMG</div>
                                                    )}
                                                    {p.images && p.images.length > 3 && (
                                                        <div className="inline-flex h-10 w-10 items-center justify-center rounded-lg bg-[#0b1326] border border-[#434655]/30 ring-2 ring-[#131b2e] text-[9px] font-bold text-[#adc6ff] relative z-10">
                                                            +{p.images.length - 3}
                                                        </div>
                                                    )}
                                                </div>
                                                <div>
                                                    <div className="font-extrabold text-[#dae2fd] text-sm leading-tight mb-1">{p.name}</div>
                                                    <div className="text-[11px] text-[#c3c5d8]/70 w-52 truncate" title={p.description || p.slug}>{p.description || p.slug}</div>
                                                    <div className="text-[9px] uppercase tracking-widest font-bold text-[#adc6ff] mt-1.5 flex items-center gap-1.5">
                                                        <Settings2 size={10} className="text-[#34d399]" /> {p.config_schema?.sections?.length || 0} Parametric Nodes
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5 font-black text-[#dae2fd] font-mono text-sm">₹{p.base_price.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                                        <td className="px-6 py-5 font-mono text-[#adc6ff] text-xs font-bold">{p.minimum_quantity} U</td>
                                        <td className="px-6 py-5">
                                        ...
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-[10px] font-bold text-[#c3c5d8] uppercase tracking-wider">{p.is_active ? 'Live' : 'Hidden'}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5 text-right">
                                            <div className="flex justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Button variant="ghost" size="icon" onClick={() => openEdit(p)} className="text-[#c3c5d8] hover:text-[#adc6ff] hover:bg-[#1f70e3]/20">
                                                    <Edit2 size={16} />
                                                </Button>
                                                <Button variant="ghost" size="icon" onClick={() => handleDelete(p.id)} className="text-[#c3c5d8] hover:text-[#ffb4ab] hover:bg-[#ffb4ab]/10">
                                                    <Trash2 size={16} />
                                                </Button>
                                            </div>
                                        </td>
            ...
            <Dialog open={showForm} onOpenChange={setShowForm}>
                <DialogContent showCloseButton={false} className="sm:max-w-[1200px] w-[95vw] h-[90vh] p-0 bg-[#0b1326] border border-[#434655]/30 shadow-2xl overflow-hidden font-['Inter'] flex flex-col">
                    ...
                    <div className="px-8 py-5 border-b border-[#434655]/20 bg-[#131b2e] shrink-0 flex items-center justify-between z-20">
                        <div>
                            <DialogTitle className="text-xl font-extrabold text-[#dae2fd]">{editingSubProduct ? "Reconfigure Node" : "Deploy Variant Node"}</DialogTitle>
                            <DialogDescription className="text-xs mt-1 text-[#c3c5d8]">
                                Instance branch of <strong className="text-[#adc6ff]">{product?.name}</strong> containing pricing rules and I/O parameters.
                            </DialogDescription>
                        </div>
                        ...
                        <div className="flex flex-col gap-6 max-w-5xl mx-auto pb-12">
                            ...
                            <div className="bg-[#131b2e] border border-[#434655]/20 rounded-2xl p-6 relative overflow-hidden">
                                ...
                                <div className="flex items-center gap-3 mb-6">
                                    <div className="bg-[#1f70e3]/10 p-2 rounded-lg"><Settings2 size={20} className="text-[#adc6ff]" /></div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    <div className="space-y-2">
                                        ...
                                        <Input className="h-10 bg-[#0b1326] border-[#434655]/40 text-[#dae2fd] focus:border-[#adc6ff] text-sm font-bold placeholder:text-[#434655]" value={formState.name} onChange={e => setFormState({ ...formState, name: e.target.value })} placeholder="e.g. Leather Cover" />
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <Input className="h-10 text-sm font-mono bg-[#0b1326] border-[#434655]/40 text-[#adc6ff] focus:border-[#adc6ff] placeholder:text-[#434655]" value={formState.slug} onChange={e => setFormState({ ...formState, slug: e.target.value })} placeholder="leather-cover" />
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                                    <div className="space-y-2">
                                        ...
                                        <div className="relative">
                                            ...
                                            <Input type="number" step="0.01" className="h-10 pl-8 font-black text-[#dae2fd] bg-[#0b1326] border-[#434655]/40 focus:border-[#adc6ff]" value={formState.base_price} onChange={e => setFormState({ ...formState, base_price: e.target.value })} placeholder="0.00" />
                                        </div>
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-bold text-[#c3c5d8] uppercase tracking-widest">MOQ Gate</label>
                                        <Input type="number" className="h-10 text-sm font-mono bg-[#0b1326] border-[#434655]/40 text-[#adc6ff]" value={formState.minimum_quantity} onChange={e => setFormState({ ...formState, minimum_quantity: e.target.value })} placeholder="100" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-bold text-[#c3c5d8] uppercase tracking-widest">Node Tag</label>
                                        <Input className="h-10 text-sm font-bold tracking-widest uppercase bg-[#0b1326] border-[#434655]/40 text-[#c3c5d8]" value={formState.type} onChange={e => setFormState({ ...formState, type: e.target.value })} placeholder="PRODUCT" />
                                    </div>
                                </div>

                                <div className="space-y-2 mb-6">
                                    <label className="text-[10px] font-bold text-[#c3c5d8] uppercase tracking-widest flex items-center justify-between">
                                        <span>Image Fleet Arrays</span>
                                        <span className="text-[9px] bg-[#434655]/30 text-[#8d90a1] px-1.5 py-0.5 rounded">OPT</span>
                                    </label>
                                    <Textarea className="h-20 text-xs font-mono bg-[#0b1326] border-[#434655]/40 text-[#8d90a1] placeholder:text-[#434655] focus:border-[#adc6ff]" value={formState.images} onChange={e => setFormState({ ...formState, images: e.target.value })} placeholder="https://cloud.com/view/1.jpg,\nhttps://cloud.com/view/back.png" />
                                </div>
                                ...
                                <div className="space-y-2 mb-6">
                                    ...
                                    <Textarea className="h-24 text-sm bg-[#0b1326] border-[#434655]/40 text-[#dae2fd] focus:border-[#adc6ff] placeholder:text-[#434655]" value={formState.description} onChange={e => setFormState({ ...formState, description: e.target.value })} placeholder="Summary configuration text..." />
                                </div>

                                <div className="flex items-center justify-between border border-[#434655]/30 bg-[#0b1326] p-4 rounded-xl">
                                    <div>
                                        <p className="font-bold text-[#dae2fd] text-sm tracking-tight text-white mb-0.5">Authorize Transmission</p>
                                        <p className="text-[11px] text-[#c3c5d8]">Enable client rendering for this node configuration.</p>
                                    </div>
                            ...
                            <div className="bg-[#131b2e] border border-[#434655]/20 rounded-2xl p-6 relative overflow-hidden">
                                ...
                                <div className="flex items-center justify-between mb-8">
                                    <div>
                                        <h3 className="text-sm font-bold tracking-widest uppercase text-[#dae2fd] flex items-center gap-3">
                                        ...
                                        <p className="text-xs text-[#c3c5d8] mt-1.5 font-medium">Inject form variables to dynamically mutate quote logic.</p>
                                    </div>
                                    <Button onClick={addSection} className="h-9 px-4 text-[10px] font-bold uppercase tracking-widest bg-transparent border border-[#adc6ff]/30 text-[#adc6ff] hover:bg-[#adc6ff] hover:text-[#001a42] transition-colors rounded-lg">
                                        <Plus size={14} className="mr-1.5" /> Attach Vector
                                    </Button>
                                </div>

                                {configSections.length === 0 ? (
                                    <div className="border border-dashed border-[#434655]/50 rounded-xl p-12 flex flex-col items-center justify-center text-center bg-[#0b1326]">
                                        <GripVertical size={24} className="text-[#434655] mb-3" />
                                        <h4 className="text-sm font-bold tracking-widest uppercase text-[#c3c5d8]">Empty Vector Schema</h4>
                                        <p className="text-xs text-[#8d90a1] mt-2 max-w-sm">Attach selection objects like color arrays or string prompts to enable dynamic pricing variables.</p>
                                    </div>
                                ) : (
                                    <div className="space-y-6">
                                        {configSections.map((sec, secIdx) => (
                                            <div key={secIdx} className="border border-[#434655]/30 rounded-xl bg-[#0b1326] overflow-hidden focus-within:border-[#fcd34d]/50 transition-colors">
                                                
                                                <div className="bg-[#171f33] px-5 py-3 border-b border-[#434655]/30 flex items-center justify-between">
                                                    <div className="flex items-center gap-4 flex-1">
                                                        <GripVertical size={16} className="text-[#434655] cursor-move" />
                                                        <Input
                                                            className="h-8 text-sm font-bold w-full max-w-sm bg-[#0b1326] border-[#434655]/40 text-[#dae2fd] focus:border-[#adc6ff] placeholder:text-[#434655]"
                                                            value={sec.label}
                                                            onChange={e => updateSectionInfo(secIdx, { label: e.target.value })}
                                                            placeholder="Display Name (e.g. Size)"
                                                        />
                                                        <select
                                                            className="h-8 rounded-md border border-[#434655]/40 bg-[#0b1326] px-3 text-xs font-bold text-[#c3c5d8] cursor-pointer outline-none focus:border-[#adc6ff] w-40"
                                                            value={sec.type}
                                                ...
                                                <div className="p-5">
                                                    <div className="flex items-center gap-4 mb-5">
                                                        <div className="flex-1 space-y-1.5">
                                                            <label className="text-[10px] font-bold uppercase tracking-widest text-[#c3c5d8]">Payload Key <span className="text-[#ffb4ab]">*</span></label>
                                                            <Input
                                                                className="h-8 text-[11px] font-mono bg-[#131b2e] border-[#434655]/30 text-[#34d399] tracking-widest focus:border-[#adc6ff]"
                                                                value={sec.key}
                                                        ...
                                                        {sec.type === "number_input" && (
                                                            <>
                                                                <div className="w-24 space-y-1.5">
                                                                    <label className="text-[10px] font-bold uppercase tracking-widest text-[#c3c5d8]">Lower Bound</label>
                                                                    <Input type="number" className="h-8 bg-[#131b2e] border-[#434655]/30 text-[#dae2fd] text-xs" placeholder="0" value={sec.min_val ?? ""} onChange={e => updateSectionInfo(secIdx, { min_val: parseInt(e.target.value) || undefined })} />
                                                                </div>
                                                                <div className="w-24 space-y-1.5">
                                                                    <label className="text-[10px] font-bold uppercase tracking-widest text-[#c3c5d8]">Upper Bound</label>
                                                                    <Input type="number" className="h-8 bg-[#131b2e] border-[#434655]/30 text-[#dae2fd] text-xs" placeholder="∞" value={sec.max_val ?? ""} onChange={e => updateSectionInfo(secIdx, { max_val: parseInt(e.target.value) || undefined })} />
                                                                </div>
                                                                <div className="w-32 space-y-1.5 border-l border-[#434655]/30 pl-4 ml-1">
                                                                    <label className="text-[10px] font-bold uppercase tracking-widest text-[#adc6ff]">Unit Multiplier</label>
                                                                    <div className="relative">
                                                                        <span className="absolute left-2.5 top-2 text-[#434655] font-bold text-xs">₹</span>
                                                                        <Input type="number" step="0.01" className="h-8 pl-6 bg-[#131b2e] border-[#434655]/30 text-[#fcd34d] text-xs font-bold" placeholder="0.0" value={sec.price_per_unit ?? ""} onChange={e => updateSectionInfo(secIdx, { price_per_unit: parseFloat(e.target.value) || undefined })} />
                                                                    </div>
                                                                </div>
                                                            </>
                                                        )}
                                                    ...
                                                    {(sec.type === "dropdown" || sec.type === "radio") && (
                                                        <div className="bg-[#131b2e] rounded-xl border border-[#434655]/20 p-4">
                                                            <div className="flex items-center justify-between mb-3 border-b border-[#434655]/20 pb-2">
                                                                ...
                                                                <Button onClick={() => addOption(secIdx)} size="sm" variant="ghost" className="h-6 text-[9px] font-bold uppercase tracking-widest text-[#adc6ff] hover:bg-[#adc6ff]/10 border border-[#adc6ff]/20">
                                                            ...
                                                            <div className="space-y-2">
                                                                {(sec.options || []).map((opt, optIdx) => (
                                                                    <div key={optIdx} className="flex flex-wrap items-center gap-2 p-2 rounded-lg bg-[#0b1326] border border-[#434655]/30 focus-within:border-[#adc6ff]/50">
                                                                        <div className="flex-1 min-w-[150px]">
                                                                            <Input
                                                                                className="h-7 text-xs font-bold bg-[#131b2e] border-transparent text-[#dae2fd] focus:border-[#434655] placeholder:text-[#434655]"
                                                                        ...
                                                                        <div className="w-32 shrink-0">
                                                                            <Input
                                                                                className="h-7 text-[10px] font-mono bg-[#131b2e] border-transparent text-[#8d90a1] focus:text-[#adc6ff] focus:border-[#434655] placeholder:text-[#434655]"
